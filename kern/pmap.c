...
boot_alloc(uint32_t n)
{
	...
	if((uint64_t)nextfree > (KERNBASE + (npages-1)*PGSIZE))
		panic("Trying to allocated memory beyond limit");
...
mem_init_mp(void)
{
	...
	for(i = 0;i<NCPU;i++){
		uintptr_t startStack = KSTACKTOP - i * (KSTKSIZE+KSTKGAP);
		boot_map_segment(boot_pml4e,startStack - KSTKSIZE,KSTKSIZE,PADDR(percpu_kstacks[i]),PTE_W|PTE_U|PTE_P);
	}
...
page_init(void)
{
	...
	//cprintf("%d, %x ",npages,page_free_list);
	pages[0].pp_ref = 1;
	pages[1].pp_link = 0 ;
	pages[0].pp_link = 0;
	...
	for(i = PPN(PADDR(BOOT_PAGE_TABLE_START));i<PPN(PADDR(BOOT_PAGE_TABLE_END));i++)
                pages[i].pp_ref = 1;
	pages[PPN(PADDR(BOOT_PAGE_TABLE_END))].pp_link = pages[PPN(PADDR(BOOT_PAGE_TABLE_START))-1].pp_link;
	
	//MPENTRY_PADDR
	i = PPN(MPENTRY_PADDR);
	pages[i].pp_ref = 1;
	
	pages[i+1].pp_link = pages[i-1].pp_link;
	...
	pages[i].pp_link = pages[PPN(IOPHYSMEM)-1].pp_link;
	//pages[i].pp_link =0;

	//Lab 6 ex 4
	//BAR0PHYSMEM
	for(i = PPN(BAR0PHYSMEM); i< PPN(BAR0LIM); i++)
			pages[i].pp_ref = 1;
	pages[PPN(BAR0LIM)].pp_link = pages[PPN(BAR0PHYSMEM) -1].pp_link;
	//page_free_list = &pages[PPN(BAR0PHYSMEM -1)];
	

	for(i = PPN(KERNBASE); i< PPN(PADDR(boot_alloc(0))); i++)
                pages[i].pp_ref = 1;
 
	pages[PPN(PADDR(boot_alloc(0)))].pp_link = pages[PPN(IOPHYSMEM)-1].pp_link;
...
page_alloc(int alloc_flags)
{
	...
	struct Page *newPage = page_free_list;
...
{
	pte_t *pte=NULL;
	struct Page *newPdpPage;
	bool isNewlyCreated = false;
	if(pml4e == 0)
		return NULL;
	
	physaddr_t pdpe = (physaddr_t)*(pml4e + PML4(va));
	pdpe_t *pdpePointer;	
	
	if(!((pdpe) & PTE_P)){
		if(create == true)
		{
			newPdpPage = page_alloc(ALLOC_ZERO);
			if(!newPdpPage) return NULL;
			newPdpPage->pp_ref++;
			pml4e[PML4(va)] = page2pa(newPdpPage);
			pml4e[PML4(va)] = pml4e[PML4(va)] | PTE_P | PTE_U | PTE_AVAIL | PTE_W;
			pdpe = (physaddr_t)pml4e[PML4(va)];
	...
	pdpePointer = KADDR( pdpe & ~0xFFF);
        pte = (pte_t *) pdpe_walk(pdpePointer,va,create);
 
	if(pte == NULL && isNewlyCreated == true){
		page_free(newPdpPage);
		pml4e[PML4(va)] = 0;
		return NULL;
	}
...
pdpe_walk(pdpe_t *pdpe,const void *va,int create){
        pte_t *pte;
        struct Page *newPdPage;
        bool isNewlyCreated = false;
	
        physaddr_t pde = (physaddr_t)*(pdpe + PDPE(va));
        pde_t *pdePointer;

        if(!((pde) & PTE_P)){
                if(create == 1)
                {
                        newPdPage = page_alloc(ALLOC_ZERO);
                        if(!newPdPage) return NULL;
                        newPdPage->pp_ref++;
                        pdpe[PDPE(va)] = page2pa(newPdPage);
                        pdpe[PDPE(va)] = pdpe[PDPE(va)] | PTE_P | PTE_U | PTE_AVAIL | PTE_W;
                        pde = (physaddr_t)pdpe[PDPE(va)];
        ...
        pdePointer = KADDR( pde & ~0xFFF);
        pte = (pte_t *) pgdir_walk(pdePointer,va,create);
...
{
        pte_t *ptxe;
        struct Page *newPtePage;
 
        physaddr_t pte = (physaddr_t)*(pgdir + PDX(va));
        pte_t *ptePointer;

        if(!((pte) & PTE_P)){
                if(create == 1)
                {
                        newPtePage = page_alloc(ALLOC_ZERO);
                        if(!newPtePage) return NULL;
                        newPtePage->pp_ref++;
                        pgdir[PDX(va)] = page2pa(newPtePage);
                        pgdir[PDX(va)] = pgdir[PDX(va)] | PTE_P | PTE_U | PTE_AVAIL | PTE_W;
                        pte = (physaddr_t)pgdir[PDX(va)];
...
//	pte =pte + PTX(va); //Pointer addition screw up
	
	ptxe = KADDR(pte & ~0xFFF);
...
boot_map_segment(pml4e_t *pml4e, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
	...
	size_t isRounded = size%PGSIZE;
	size_t noOfPages = size/PGSIZE;
	
	if(isRounded > 0 ) noOfPages++;
	...
	{
		pte = pml4e_walk(pml4e, (void *)va, create);
		if(pte == NULL) 
			cprintf("Could not create page!!");
...
{
	int create = 0; 	//Never create if does not exist
	pte_t *pte = pml4e_walk(pml4e,va,create);
	...
	//	if(pte_store != 0)
	    ...
	    if((*pte) & PTE_P){	
		physaddr_t physicalPageBase = *pte & ~0xFFF;
		return pa2page(physicalPageBase);
		}
	}
...
{
	pte_t *pteToPageCheck = NULL;
	struct Page *page;
	
	page = page_lookup(pml4e,va,&pteToPageCheck);
	
	if(page!=NULL){
		page_decref(page);
		*pteToPageCheck = 0;
		tlb_invalidate(pml4e,va); //Remember :: The function does not use pml4e
	}
	else return;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	boot_map_segment(boot_pml4e,startAddr,size,pa,PTE_PCD|PTE_PWT|PTE_W|PTE_P);
...
{
	// LAB 3: Your code here.
	const void *startVa  = ROUNDDOWN(va, PGSIZE);
	const void *endVa = ROUNDUP((va+len), PGSIZE);
	
	int i, numPages = (endVa-startVa)/PGSIZE;
	
	for(i=0; i<numPages; i++)
	{
		if(startVa >= (void*)ULIM)
		{
			if( i == 0)
				user_mem_check_addr = (uintptr_t)va;
			else
				user_mem_check_addr = (uintptr_t)startVa;
			
			//cprintf("Fault found at %x\n",user_mem_check_addr);
			
			return -E_FAULT;
		}
		
		pte_t *pte = pml4e_walk(env->env_pml4e, startVa, 0);
