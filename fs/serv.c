...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	int numRead = req->req_n;
	if(req->req_n > PGSIZE) numRead = PGSIZE;
	
	int fileId = req->req_fileid;
	struct OpenFile *openedFile;

	int lookUp = openfile_lookup( envid, (uint32_t)fileId, &openedFile);

	if(lookUp < 0) return lookUp;

	if( openedFile == NULL || openedFile->o_fd == NULL) return -E_INVAL;
	//if(openedFile.o_mode != 0) return -E_INVAL;

	//char returnBuffer[PGSIZE];
	
	ssize_t numReadChars = file_read( openedFile->o_file, ret->ret_buf, numRead , openedFile->o_fd->fd_offset );

	if(numReadChars < 0) return numReadChars;

	//ret->ret_buf = returnBuffer;

	openedFile->o_fd->fd_offset += numReadChars;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	// LAB 5: Your code here.
	int write;
	int numWriteChars = req->req_n;
	//int writeMaxSize = PGSIZE - sizeof(int) - sizeof(size_t);

	struct OpenFile *openFile = NULL;
	if(numWriteChars > PGSIZE) numWriteChars = PGSIZE;

	write = openfile_lookup(envid,req->req_fileid,&openFile);

	if(openFile == NULL || openFile->o_fd == NULL) return -E_INVAL;
	if(write<0) return -write;

	write = file_write(openFile->o_file,req->req_buf,numWriteChars,openFile->o_fd->fd_offset);

	if(write < 0) return -write;

	openFile->o_fd->fd_offset += numWriteChars;
