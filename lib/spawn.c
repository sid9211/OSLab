...
copy_shared_pages(envid_t child)
{
	...
	for (vAddr = (uint8_t *)0 ; vAddr < (uint8_t *)UTOP; vAddr += PGSIZE ){
		...
		if ( (vpml4e[VPML4E(vAddr)]&PTE_P) && (vpde[VPDPE(vAddr)]&PTE_P) && (vpd[VPD(vAddr)] & PTE_P) && (vpt[VPN(vAddr)] & PTE_P) ){
			
			pageNum = VPN(vAddr); //Get page number

			if ( vpt[pageNum] & PTE_SHARE ) {
				if( (r = sys_page_map(0, (void*)vAddr, child, (void*)vAddr,   //Jst fr cmnt
					( PGOFF(vpt[pageNum]) & PTE_USER) | PTE_SHARE) ) < 0)
